...
它们之间的层次关系是：启动类加载器是扩展类加载器的上级类加载器，扩展类加载器是应用类加载器的上级类加载器，应用类加载器是自定义类加载器的上级类加载器。
...
本地方法栈和虚拟机栈的作用类似，只不过本地方法栈是在执行本地方法时使用的。与虚拟机栈一样，本地方法栈也会在栈深度溢出或者栈扩展失败时分别抛出StackOverflowError和OutOfMemoryError异常。
...
* 对象头
  ...
  对象头包含三部分内容，一部分是和对象运行时数据相关的Mark Word，长度在32位虚拟机和64位虚拟机（未开启压缩指针）上分别是32bit和64bit；另一部分是类型指针，指向该对象的类型元数据的指针，通过这个指针可以确定它是哪个类的实例，未开启类型指针压缩时长度为8byte；如果对象是一个数组，还有一个记录数组长度的数据，大小为4byte。
...
使用直接指针来访问最大的好处就是速度更快，它节省了一次指针定位的时间开销，由于对象访问在Java中非常频繁，因此这类开销积少成多也是一项极为可观的执行成本，Hotspot使用的直接指针的方式进行对象访问。
...
对于int的包装类型Integer，占用16字节，而int只占用4字节。
对于long类型的包装类型Long，占用24字节，而long只占用8字节。
