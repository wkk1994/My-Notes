...
`AbstractApplicationContext#registerBeanPostProcessors`方法，底层依赖`PostProcessorRegistrationDelegate#registerBeanPostProcessors(org.springframework.beans.factory.config.ConfigurableListableBeanFactory, org.springframework.context.support.AbstractApplicationContext)`，注册BeanPostProcessor阶段和执行BeanFactoryPostProcessors方法类似。主要步骤：

* 注册BeanPostProcessorChecker

  BeanPostProcessorChecker中有数字记录当前它注册为BeanPostProcessor时系统中的BeanPostProcessor实例数量，在`postProcessAfterInitialization`方法，它会检查记录的BeanPostProcessor实例数量和当前BeanFactory中的是否一致，如果不一致就认为，当前的Bean在实例化时并没有被所有的BeanPostPorcessor所处理，然后输出日志。
